...
let
  ...
  mkVolumeDef =
    ...
    if host != null then
      ...
      "${cfg.volumeBaseDir}/${container.volumeSubDir}/${name}:${inner}";

  shouldPreStart = _name: container: container.pullOnStart;
  mkPreStart =
    name: container:
    nameValuePair "${cfg.backend}-${name}" {
  ...
  mkNetworkService =
    _name: container:
    let
      package = if cfg.backend == "docker" then pkgs.docker else pkgs.podman;
    in
    nameValuePair "${cfg.backend}-net-${container.network}" {
  ...
  mkPodService =
    ...
    let
      package =
        if cfg.backend == "podman" then pkgs.podman else throw "mkPodService only supports podman";
    ...
    nameValuePair "${cfg.backend}-pod-${name}" {
      ...
      serviceConfig = {
        ...
        ExecStart =
          ...
          in
          "${package}/bin/${cfg.backend} pod create ${concatStringsSep " " args} ${name}";
  ...
  allContainers =
    let
      mkPodContainer =
        podName: pod: containerName: container:
